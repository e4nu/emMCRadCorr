...
void treeProducer_simulation::Loop() {
  ...
  TFile* acc_e = new TFile("data/AcceptanceMap_e_TH3D.root");
  TH3D* h3_e = (TH3D*)(acc_e->Get("h3"));
  TFile* acc_p = new TFile("data/AcceptanceMap_p_TH3D.root");
  TH3D* h3_p = (TH3D*)(acc_p->Get("h3"));

  TString out_file_name = "rad_w_acc";
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    std::cout << " AccWeight_p " << AccWeight_p << " AccWeight_e " << AccWeight_e << std::endl;
