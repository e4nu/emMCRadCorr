...
void ExtDataFig2() {
	...
	std::vector<int> Colors; Colors.push_back(kBlack); Colors.push_back(kBlack); Colors.push_back(kBlue); Colors.push_back(kGreen); Colors.push_back(kRed);

	std::vector<TString> FSIModel; 
        FSIModel.push_back("Data");
	FSIModel.push_back("norad");
	FSIModel.push_back("AdiFixed");
        FSIModel.push_back("simple_peaking");//1H_4_325_simple_Plots_FSI_em.root 
	std::vector<TString> FSILabel; 
        FSILabel.push_back("Data"); 
        FSILabel.push_back("Default GENIE"); 
	...
	for (int WhichEnergy = 0; WhichEnergy < NEnergies; WhichEnergy ++) {
		...
		for (int WhichPlot = 0; WhichPlot < NPlots; WhichPlot ++) {
			...
			if(WhichFSIModel==3)Plots[WhichFSIModel]->SetLineStyle(2);
			...
			double ScalingFactor = 1./Plots[WhichFSIModel]->Integral();//Plots[0]->Integral() / Plots[WhichFSIModel]->Integral();
